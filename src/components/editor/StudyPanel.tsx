      ...
      <div className="flex-1 overflow-y-auto p-4">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-4 bg-white border border-[#D0A348]/20">
            <TabsTrigger value="concordance" className="data-[state=active]:bg-[#082C50] data-[state=active]:text-white">Concordance</TabsTrigger>
            <TabsTrigger value="commentary" className="data-[state=active]:bg-[#082C50] data-[state=active]:text-white">Commentary</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="concordance" className="space-y-4">
            <div className="flex gap-2">
              <Input
                ...
                className="bg-white border-[#D0A348]/30 focus-visible:ring-[#082C50]"
              />
              <Button onClick={searchConcordance} className="bg-[#082C50] hover:bg-[#082C50]/90 text-white">
                <Search className="h-4 w-4" />
              </Button>
            </div>

            {isLoading && (
              <div className="py-12 flex flex-col items-center justify-center text-muted-foreground space-y-4 text-[#082C50]">
            ...
            {concordanceResults && !isLoading && (
              <div className="space-y-4">
                <div className="bg-white p-4 rounded-md border border-[#D0A348]/20 shadow-sm">
                  <h3 className="font-serif text-lg text-[#082C50] font-semibold">Original Word: {concordanceResults.originalWord}</h3>
                ...
                <div className="space-y-3">
                  {concordanceResults.occurrences?.map((occ: any, i: number) => (
                    ...
                    >
                      <h4 className="font-semibold text-[#082C50] group-hover:text-[#D0A348] transition-colors">{occ.reference}</h4>
          ...
          <TabsContent value="commentary" className="space-y-4">
            <div className="flex gap-2">
              <Input
                ...
                className="bg-white border-[#D0A348]/30 focus-visible:ring-[#082C50]"
              />
              <Button onClick={searchCommentary} className="bg-[#082C50] hover:bg-[#082C50]/90 text-white">
                <Search className="h-4 w-4" />
              </Button>
            </div>
            ...
            {commentaryResults && !isLoading && (
              <div className="space-y-4 pb-8">
                <div className="bg-white p-4 rounded-md border border-[#D0A348]/20 shadow-sm space-y-2">
                  <h3 className="font-serif text-lg text-[#082C50] font-semibold border-b pb-2 mb-2">Historical Context</h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{commentaryResults.historicalContext}</p>
                </div>

                <div className="bg-white p-4 rounded-md border border-[#D0A348]/20 shadow-sm space-y-2">
                  <h3 className="font-serif text-lg text-[#082C50] font-semibold border-b pb-2 mb-2">Key Themes</h3>
                ...
                <div className="bg-white p-4 rounded-md border border-[#D0A348]/20 shadow-sm space-y-2">
                  <h3 className="font-serif text-lg text-[#082C50] font-semibold border-b pb-2 mb-2">Original Language</h3>
                  <p className="text-sm text-gray-700 leading-relaxed">{commentaryResults.originalLanguage}</p>
                </div>

                <div className="bg-white p-4 rounded-md border border-[#D0A348]/20 shadow-sm space-y-2">
                  <h3 className="font-serif text-lg text-[#082C50] font-semibold border-b pb-2 mb-2">Cross-References</h3>
                  <ul className="space-y-1">
                    {commentaryResults.crossReferences?.map((ref: string, i: number) => (
                      <li key={i} className="text-sm text-[#082C50] hover:text-[#D0A348] cursor-pointer transition-colors flex items-center gap-2">
                ...
                <div className="bg-[#082C50] p-4 rounded-md border border-[#082C50] shadow-sm space-y-2 text-white">
